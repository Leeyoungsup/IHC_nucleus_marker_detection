...
def visualize_ground_truth_and_prediction_separately(model, dataset, idx=0, conf_threshold=0.5, iou_threshold=0.3, epoch=None, save_dir=None):
    ...
    for i in range(len(cls)):
        ...
        if class_id == 0: #negative tumor cell
            color = 'green'
        elif class_id == 1: #positive tumor cell (class 1)
            color = 'yellow'
        elif class_id == 2: #positive tumor cell - moderate
            color = 'blue'
        elif class_id == 3: #positive tumor cell - strong
            color = 'red'
        # 중심점 표시
        # 중심점 좌표 계산
    ...
    with torch.no_grad():
        ...
        if len(results[0]) > 0:
            for *xyxy, conf, cls_id in results[0]:
                ...
                if cls_id.item() == 0: #negative tumor cell
                    color = 'green'
                elif cls_id.item() == 1: #positive tumor cell (class 1)
                    color = 'yellow'
                elif cls_id.item() == 2: #positive tumor cell - moderate
                    color = 'blue'
                elif cls_id.item() == 3: #positive tumor cell - strong
                    color = 'red'
                center_x = (x1 + x2)//2
                center_y = (y1 + y2)//2
    ...
    # 범례 추가
    legend_elements = [
        patches.Patch(color='green', label='negative tumor cell'),
        patches.Patch(color='yellow', label='positive tumor cell (class 1)'),
        patches.Patch(color='blue', label='positive tumor cell - moderate (class 2)'),
        patches.Patch(color='red', label='positive tumor cell - strong (class 3)'),
    ]
